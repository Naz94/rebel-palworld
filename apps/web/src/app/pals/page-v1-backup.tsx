...
function getPracticalBaseCandidates(
  ...
  return pals
    .map((entry) => {
      ...
      const activeRoles =
        entry.score.workRoles.filter(
          (item) =>
            item.level > 0,
        ).length;

      const passiveAdjustment =
        getBasePassiveAdjustment(
          entry,
          job,
        );
...
function getBaseCandidateReasons(
  ...
  if (
    ...
    reasons.push(
      `${job} Lv.${candidate.role?.level} powerhouse`,
    );
  } else if (
    candidate.role?.level
  ) {
    reasons.push(
      `${job} Lv.${candidate.role.level}`,
    );
  }
